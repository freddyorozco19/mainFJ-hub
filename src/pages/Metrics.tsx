...
function AgentRow({ name, icon, tokens, cost, requests, pct }: {
  name: string; icon: string; tokens: number; cost: number; requests: number; pct: number
}) {
  return (
    <div className="flex items-center gap-4 py-3 border-b border-border/50 last:border-0">
      ...
      <div className="text-right flex-shrink-0">
        ...
        <p className="text-[10px] text-slate-600">{requests} req · ${cost.toFixed(4)}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex-1 p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Consumo y costos de la API de Claude</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        {agents.length === 0 ? (
          <p className="text-sm text-slate-600 py-4 text-center">Sin datos aún — empieza a chatear</p>
        ) : (
          <div>
            {agents.map(a => (
              <AgentRow
                ...
                cost={a.totalCost}
                requests={0}
                pct={(a.totalTokens / maxTokens) * 100}
              />
            ))}
          </div>
        )}
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <p className="text-xs text-slate-600 mb-6">Tokens consumidos por día</p>
        <div className="flex items-end gap-2 h-24">
          {Array.from({ length: 14 }, (_, i) => {
            const h = Math.random() * 80 + 5
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full bg-primary/30 rounded-t-sm hover:bg-primary/60 transition-colors cursor-pointer" style={{ height: `${h}%` }} />
              </div>
            )
          })}
        </div>
        <p className="text-[10px] text-slate-700 text-center mt-3">Últimos 14 días</p>
      </div>
    </div>
  )
}
